...
class DataScalars extends PolymerElement {
  static get template() {
    ...
    <div id="scalar" class\$="content [[_getErrorClass(data)]]" editing\$="[[editing]]" disabled\$="[[_disabled]]" title="[[_title]]">

      <!-- Icon -->
      <template is="dom-if" if="[[_isDataOfType(data, 'icon')]]">
        <template is="dom-if" if="[[editing]]">
          <input class\$="input element [[_elementType]] [[_elementVariant]]" value\$="[[_value]]" title="[[_title]]" disabled="[[_disabled]]" placeholder="[[placeholder]]" on-change="_onChange" on-keyup="_onKeyup" on-keydown="_onKeydown">
        </template>
        <template is="dom-if" if="[[!editing]]">
      ...
      <template is="dom-if" if="[[editing]]">
        ...
        <template is="dom-if" if="[[_isDataOfTypePrimitive(data)]]">
          <template is="dom-if" if="[[!_disabled]]">
            <input class\$="input element [[_elementType]] [[_elementVariant]]" type\$="[[_getType(data)]]" value\$="[[_value]]" title="[[_title]]" disabled="[[_disabled]]" placeholder="[[placeholder]]" on-change="_onChange" on-keyup="_onKeyup" on-keydown="_onKeydown">
          </template>
          <template is="dom-if" if="[[_disabled]]">
            [[_formatValue(data, _value, _suggestions)]]
          </template>
        ...
        <template is="dom-if" if="[[_isDataOfTypeDate(data)]]">
          <template is="dom-if" if="[[!_disabled]]">
            <input class\$="input element [[_elementType]] [[_elementVariant]]" type\$="[[_parseHTMLDateType(data)]]" value\$="[[_formatDatetime(data, _value, 'true')]]" title="[[_title]]" disabled="[[_disabled]]" placeholder="[[placeholder]]" on-change="_onChange" on-keyup="_onKeyup" on-keydown="_onKeydown">
          </template>
          <template is="dom-if" if="[[_disabled]]">
            [[_formatValue(data, _value, _suggestions)]]
          </template>
        ...
        <template is="dom-if" if="[[_isDataOfType(data, 'dropdown')]]">
          <template is="dom-if" if="[[!_disabled]]">
            <vaadin-combo-box-light class\$="input element dropdown [[_elementType]] [[_elementVariant]]" placeholder="[[placeholder]]" items="[[_suggestions]]" value="[[_getLabelFromValue(_suggestions, _value)]]" attr-for-value="bind-value" disabled="[[_disabled]]" allow-custom-value="[[_allowCustomValue]]" selected-item="{{_dropdownSelectedItem}}">
              <template>
                <div style="font-size: 10px; font-family:'Roboto';">[[item.label]] ([[item.value]])</div>
              </template>
              <iron-input>
                <input placeholder="[[placeholder]]" disabled\$="[[_disabled]]" on-keyup="_onKeyup" on-keydown="_onKeydown">
              </iron-input>
            </vaadin-combo-box-light>
          </template>
        </template>
        ...
        <template is="dom-if" if="[[_isDataOfType(data, 'checkbox')]]">
          <template is="dom-if" if="[[!_disabled]]">
            <select class\$="input element checkbox [[_elementType]] [[_elementVariant]]" disabled="[[_disabled]]" on-change="_onTick" value="[[_value]]" on-keyup="_onKeyup">
              <option>unset</option>
